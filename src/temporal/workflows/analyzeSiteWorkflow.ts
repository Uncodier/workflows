 ...
 */
function extractSettingsUpdateFromDeepResearch(deepResearchResult: any, existingSettings: any): any {
  ...
  if (company.products && Array.isArray(company.products) && company.products.length > 0) {
    const mappedProducts = company.products
      ...
      .map((p: any) => ({
        ...
        features: Array.isArray(p.features) 
          ? p.features.filter((f: any) => f && typeof f === 'string' && f.trim() !== '').map((f: any) => f.trim())
          : [],
    ...
    if (mappedProducts.length > 0 && isEmptyField(existingSettings.products)) {
      ...
      updateCount++;
      console.log(`   ✅ Products: ${mappedProducts.length} items - ${mappedProducts.map((p: any) => p.name).join(', ')}`);
    }
  }
  ...
  if (company.services && Array.isArray(company.services) && company.services.length > 0) {
    ...
    if (mappedServices.length > 0 && isEmptyField(existingSettings.services)) {
      ...
      updateCount++;
      console.log(`   ✅ Services: ${mappedServices.length} items - ${mappedServices.map((s: any) => s.name).join(', ')}`);
    }
  }
  ...
  if (company.swot && typeof company.swot === 'object' && Object.keys(company.swot).length > 0) {
    ...
    const mappedSWOT = {
      strengths: Array.isArray(swot.strengths) 
        ? swot.strengths.filter((s: any) => s && typeof s === 'string' && s.trim() !== '').map((s: any) => s.trim())
        : [],
      weaknesses: Array.isArray(swot.weaknesses) 
        ? swot.weaknesses.filter((w: any) => w && typeof w === 'string' && w.trim() !== '').map((w: any) => w.trim())
        : [],
      opportunities: Array.isArray(swot.opportunities) 
        ? swot.opportunities.filter((o: any) => o && typeof o === 'string' && o.trim() !== '').map((o: any) => o.trim())
        : [],
      threats: Array.isArray(swot.threats) 
        ? swot.threats.filter((t: any) => t && typeof t === 'string' && t.trim() !== '').map((t: any) => t.trim())
        : []
    };
  ...
  if (company.competitors && Array.isArray(company.competitors) && company.competitors.length > 0) {
    const mappedCompetitors = company.competitors
      ...
      .map((c: any) => ({
        ...
        key_differentiators: Array.isArray(c.key_differentiators) 
          ? c.key_differentiators.filter((d: any) => d && typeof d === 'string' && d.trim() !== '').map((d: any) => d.trim())
          : [],
        strengths: Array.isArray(c.strengths) 
          ? c.strengths.filter((s: any) => s && typeof s === 'string' && s.trim() !== '').map((s: any) => s.trim())
          : [],
        weaknesses: Array.isArray(c.weaknesses) 
          ? c.weaknesses.filter((w: any) => w && typeof w === 'string' && w.trim() !== '').map((w: any) => w.trim())
          : []
      }));
    ...
    if (mappedCompetitors.length > 0 && isEmptyField(existingSettings.competitors)) {
      ...
      updateCount++;
      console.log(`   ✅ Competitors: ${mappedCompetitors.length} items - ${mappedCompetitors.map((c: any) => c.name).join(', ')}`);
    }
  }
  ...
  if (company.team_members && Array.isArray(company.team_members) && company.team_members.length > 0) {
    const mappedMembers = company.team_members
      ...
      .map((m: any) => ({
        ...
        responsibilities: Array.isArray(m.responsibilities) 
          ? m.responsibilities.filter((r: any) => r && typeof r === 'string' && r.trim() !== '').map((r: any) => r.trim())
          : [],
    ...
    if (mappedMembers.length > 0 && isEmptyField(existingSettings.team_members)) {
      ...
      updateCount++;
      console.log(`   ✅ Team members: ${mappedMembers.length} items - ${mappedMembers.map((m: any) => m.name).join(', ')}`);
    }
  }
  ...
  if (company.team_roles && Array.isArray(company.team_roles) && company.team_roles.length > 0) {
    const mappedRoles = company.team_roles
      .filter((r: any) => r && r.title && r.title.trim() !== '')
      .map((r: any) => ({
        title: r.title?.trim() || '',
        department: r.department?.trim() || null,
        responsibilities: Array.isArray(r.responsibilities) 
          ? r.responsibilities.filter((resp: any) => resp && typeof resp === 'string' && resp.trim() !== '').map((resp: any) => resp.trim())
          : [],
        requirements: Array.isArray(r.requirements) 
          ? r.requirements.filter((req: any) => req && typeof req === 'string' && req.trim() !== '').map((req: any) => req.trim())
          : [],
    ...
    if (mappedRoles.length > 0 && isEmptyField(existingSettings.team_roles)) {
      ...
      updateCount++;
      console.log(`   ✅ Team roles: ${mappedRoles.length} items - ${mappedRoles.map((r: any) => r.title).join(', ')}`);
    }
  }
  ...
  if (company.org_structure && typeof company.org_structure === 'object' && Object.keys(company.org_structure).length > 0) {
    ...
    const mappedOrgStructure = {
      departments: Array.isArray(orgStructure.departments) 
        ? orgStructure.departments.filter((d: any) => d && typeof d === 'string' && d.trim() !== '').map((d: any) => d.trim())
        : [],
  ...
  if (company.locations && Array.isArray(company.locations) && company.locations.length > 0) {
    const mappedLocations = company.locations
      .filter((l: any) => l && l.name && l.name.trim() !== '')
      .map((l: any) => ({
        name: l.name?.trim() || '',
  ...
  if (company.business_hours && Array.isArray(company.business_hours) && company.business_hours.length > 0) {
    const mappedHours = company.business_hours
      .filter((h: any) => h && h.name && h.timezone)
      .map((h: any) => ({
        name: h.name?.trim() || '',
  ...
  if (company.marketing_channels && Array.isArray(company.marketing_channels) && company.marketing_channels.length > 0) {
    const mappedChannels = company.marketing_channels
      .filter((c: any) => c && c.channel && c.channel.trim() !== '')
      .map((c: any) => ({
        ...
        performance_metrics: Array.isArray(c.performance_metrics) 
          ? c.performance_metrics.filter((m: any) => m && typeof m === 'string' && m.trim() !== '').map((m: any) => m.trim())
          : []
      }));
  ...
  if (company.social_media && Array.isArray(company.social_media) && company.social_media.length > 0) {
    const mappedSocial = company.social_media
      .filter((s: any) => s && s.platform && s.platform.trim() !== '')
      .map((s: any) => ({
  ...
  if (company.channels && Array.isArray(company.channels) && company.channels.length > 0) {
    const mappedChannels = company.channels
      .filter((c: any) => c && c.type && c.type.trim() !== '')
      .map((c: any) => ({
        type: c.type?.trim() || '',
...
export async function analyzeSiteWorkflow(
  ...
  try {
    ...
    if (settingsUpdates) {
      ...
      if (updateSettingsResult.success) {
        ...
        if (settingsUpdates.products && settingsUpdates.products.length > 0) {
          settingsInsights.push(`${settingsUpdates.products.length} products identified and catalogued: ${settingsUpdates.products.map((p: any) => p.name).join(', ')}`);
        }
        if (settingsUpdates.services && settingsUpdates.services.length > 0) {
          settingsInsights.push(`${settingsUpdates.services.length} services identified and catalogued: ${settingsUpdates.services.map((s: any) => s.name).join(', ')}`);
        }
        ...
        if (settingsUpdates.competitors && settingsUpdates.competitors.length > 0) {
          settingsInsights.push(`${settingsUpdates.competitors.length} key competitors identified: ${settingsUpdates.competitors.map((c: any) => c.name).join(', ')}`);
        }
        if (settingsUpdates.team_members && settingsUpdates.team_members.length > 0) {
          settingsInsights.push(`${settingsUpdates.team_members.length} team members profiled: ${settingsUpdates.team_members.map((m: any) => m.name).join(', ')}`);
        }
        if (settingsUpdates.team_roles && settingsUpdates.team_roles.length > 0) {
          settingsInsights.push(`${settingsUpdates.team_roles.length} organizational roles defined: ${settingsUpdates.team_roles.map((r: any) => r.title).join(', ')}`);
        }
    ...
    const criticalErrorsExist = errors.some(error => 
      error.includes('Failed to get site information') || 
    ...
    if (!criticalErrorsExist) {
      ...
      const allInsights = allInsightStrings.map((insight: string, index: number) => {
        // Determinar el tipo basado en el contenido - solo usar tipos permitidos por la API
        let type: 'finding' | 'change' | 'recommendation' | 'alert' = 'finding';
