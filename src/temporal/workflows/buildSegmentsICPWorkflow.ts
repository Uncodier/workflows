...
export interface BuildSegmentsICPOptions {
  siteId?: string;                    // Site ID (legacy)
  site_id: string;                    // Required: Site ID
  userId?: string;
  segmentIds?: string[];              // Optional: specific segment IDs to process
  timeout?: number;
  aiProvider?: 'openai' | 'anthropic' | 'gemini';
  aiModel?: string;
}

export interface BuildSegmentsICPResult {
  success: boolean;
  siteId: string;
  siteName?: string;
  siteUrl?: string;
  icpSegmentsBuilt: number;
  segmentIds: string[];
  segments?: any[];
  analysis?: any;
  errors: string[];
...
export interface BuildSingleSegmentICPOptions {
  siteId: string;
  userId?: string;
  segmentId: string;
  segmentName?: string;
...
export interface BuildSingleSegmentICPResult {
  success: boolean;
  siteId: string;
  segmentId: string;
  segmentName?: string;
 ...
 */
export async function buildSingleSegmentICPWorkflow(
  options: BuildSingleSegmentICPOptions
): Promise<BuildSingleSegmentICPResult> {
  ...
  try {
    ...
    const result: BuildSingleSegmentICPResult = {
      success: true,
      siteId,
      segmentId,
      segmentName,
 ...
 */
export async function buildSegmentsICPWorkflow(
  options: BuildSegmentsICPOptions
): Promise<BuildSegmentsICPResult> {
  ...
  const childWorkflowResults: BuildSingleSegmentICPResult[] = [];
  ...
  try {
    ...
    for (let i = 0; i < segmentIds.length; i++) {
      ...
      try {
        ...
        const childWorkflowHandle = await startChild(buildSingleSegmentICPWorkflow, {
    ...
    const executionTime = `${((Date.now() - startTime) / 1000).toFixed(2)}s`;
    const result: BuildSegmentsICPResult = {
