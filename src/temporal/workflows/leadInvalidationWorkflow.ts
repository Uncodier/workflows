...
const { 
  ...
  deleteLeadConversationsActivity,
  updateLeadActivity,
...
export interface LeadInvalidationResult {
  ...
  completedAt: string;
  conversationsDeleted?: number;      // Number of conversations deleted during hard invalidation
  messagesDeleted?: number;           // Number of messages deleted during hard invalidation
  hardInvalidation?: boolean;         // Whether this was a hard invalidation (no alternative contact)
}
...
export async function leadInvalidationWorkflow(
  ...
  try {
    ...
    const leadStatus: string | undefined = (lead as any).status;
    ...
    try {
      ...
      if (tasksInfo.success && tasksInfo.existingTasks && tasksInfo.existingTasks.length > 0) {
        ...
        hasConsiderationOrLaterTasks = tasksInfo.existingTasks.some((t: any) => considerationStages.has(t.stage));
        if (hasConsiderationOrLaterTasks) {
          console.log(`🛑 Lead has tasks in consideration+ stages. Stages found: ${tasksInfo.existingTasks.map((t: any) => t.stage).join(', ')}`);
        }
      }
    } catch (taskCheckErr) {
