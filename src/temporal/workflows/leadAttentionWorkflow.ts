...
const { 
  checkExistingLeadNotificationActivity,
  getLeadActivity,
  leadAttentionActivity 
} = proxyActivities<typeof activities>({
  startToCloseTimeout: ACTIVITY_TIMEOUTS.DEFAULT, // ✅ Using centralized config (2 minutes)
  retry: RETRY_POLICIES.NETWORK, // ✅ Using appropriate retry policy for network operations
});

// Note: Now we call each activity separately in the workflow for better visibility

/**
 * Workflow parameters interface
 */
export interface LeadAttentionParams {
  lead_id: string;
 ...
 */
export async function leadAttentionWorkflow(params: LeadAttentionParams): Promise<LeadAttentionResult> {
  ...
  try {
    ...
    const assigneeId = lead.assignee_id || (lead as any).assignee_id;
    ...
    return {
      ...
      data: {
        ...
        leadInfo: {
          id: lead.id,
          name: (lead.name || lead.email || 'Unknown Lead') as string,
          company: lead.company || lead.company_name
        }
      },
      error: result.error,
