...
export interface LeadGenerationDomainSearchOptions {
  domains: string[]; // Array of domains to search
  page: number; // 0-based
  page_size: number;
  site_id: string;
  userId: string;
  company?: any; // Company data for context
  segmentId?: string; // Optional segment ID
}

export interface LeadGenerationDomainSearchResult {
  success: boolean;
 ...
 */
export async function leadGenerationDomainSearchWorkflow(
  options: LeadGenerationDomainSearchOptions
): Promise<LeadGenerationDomainSearchResult> {
