...
export interface IdealClientProfileMiningOptions {
  icp_mining_id?: string; // id en tabla icp_mining; si no viene o es 'ALL', procesa pendientes
  site_id: string;
  userId?: string;
 ...
 */
function calculateStartingPage(processedTargets: number, pageSize: number): number {
...
export async function idealClientProfileMiningWorkflow(options: IdealClientProfileMiningOptions): Promise<IdealClientProfileMiningResult> {
  ...
  const processSingle = async (icp: any) => {
    ...
    while (currentPage <= maxPages && hasMorePages && foundMatches < targetLeadsWithEmail) {
      ...
      await logWorkflowExecutionActivity({
        ...
        output: { 
          apiResponseDebug: {
            ...
            dataKeys: (pageRes as any).data ? Object.keys((pageRes as any).data) : [],
            searchResultsCount: (pageRes as any).data?.search_results?.length || 0,
      ...
      const searchResults = (pageRes as any).data?.search_results || [];
      ...
      if (persons.length > 0) {
        const samplePersons = persons.slice(0, 3).map((p: any) => ({
      ...
      if (validatedEmail) {
        ...
        try {
          ...
          const leadResult = await createSingleLead(
            leadData,
            options.site_id,
            actualUserId, // Use the actual user_id from site information
  ...
  if (!isBatch) {
    ...
    if (!icpRes.success || !icpRes.icp) {
      ...
      errors.push(msg);
      await markIcpMiningCompletedActivity({ id: options.icp_mining_id as string, failed: true, last_error: msg });
      return { success: false, icp_mining_id: options.icp_mining_id as string, processed: 0, foundMatches: 0, errors };
    }
