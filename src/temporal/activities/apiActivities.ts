...
export async function validateContactInformation(request: {
  ...
  try {
    ...
    if (isEmailUsable) {
      ...
      reason = `Email is invalid and not deliverable (${emailData?.result || 'unknown'})`;
    } else if (!isValid) {
      reason = `Email is invalid (${emailData?.result || 'unknown'})`;
    } else if (!isDeliverable) {
      reason = `Email is not deliverable (${emailData?.result || 'unknown'})`;
    } else {
      reason = `Email validation failed (${emailData?.result || 'unknown'})`;
    }
    ...
    return {
      ...
      suggested_correction: emailData?.suggested_correction,
      execution_time: emailData?.execution_time,
 ...
 */
export async function companyGenericContactGenerationActivity(request: {
  domain: string;
  context: string;
  site_id: string;
}): Promise<{
  ...
  try {
    const response = await apiService.post('/api/agents/dataAnalyst/companyContactGeneration', {
      domain: normalizedDomain,
      context,
      site_id
    });
