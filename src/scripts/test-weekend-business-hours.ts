 ...
 */
async function evaluateWeekendBusinessHours(dayOfWeek: number, mockCurrentTime: Date): Promise<{
  ...
  const openSites = sitesWithBusinessHours.filter(setting => {
    ...
    if (Array.isArray(businessHours) && businessHours.length > 0) {
      ...
      if (firstSchedule && firstSchedule.days && (firstSchedule.days as any)[today]) {
        const dayConfig = (firstSchedule.days as any)[today];
        todayHours = {
  ...
  return {
    ...
    nextExecutionTime,
       ...
       siteId: site.site_id,
       businessHours: site.business_hours?.[0]?.days ? (site.business_hours[0].days as any)[today] : null
     }))
  };
}
...
export { testWeekendBusinessHours }; 