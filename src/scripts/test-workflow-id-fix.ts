...
import { activityPrioritizationEngineWorkflow } from '../temporal/workflows/activityPrioritizationEngineWorkflow';
...
async function testWorkflowIdFix() {
  ...
  try {
    ...
    const workflowAlreadyStartedErrors = results.filter(r => 
      !r.success && r.error?.includes('Workflow execution already started')
    );
      ...
      successfulResults.forEach(result => {
        ...
        if (operationsResult?.individualSchedules) {
          ...
          if (operationsResult.scheduleDetails) {
            const duplicateWorkflows = operationsResult.scheduleDetails.filter((detail: any) => 
              detail.error?.includes('Workflow already exists')
            );
