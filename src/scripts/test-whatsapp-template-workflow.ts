...
import { sendWhatsappFromAgent } from '../temporal/workflows/sendWhatsappFromAgentWorkflow';
...
async function testWhatsAppTemplateWorkflow() {
  ...
  try {
    ...
    const workflowId1 = `test-whatsapp-direct-${nanoid()}`;
    
    const handle1 = await client.workflow.start(sendWhatsappFromAgent, {
      args: [baseParams],
      taskQueue: 'workflows',
    ...
    const workflowId2 = `test-whatsapp-template-${nanoid()}`;
    
    const handle2 = await client.workflow.start(sendWhatsappFromAgent, {
      args: [baseParams],
      taskQueue: 'workflows',
...
async function testParameterValidation() {
  ...
  try {
    ...
    const workflowId = `test-validation-${nanoid()}`;
    
    const handle = await client.workflow.start(sendWhatsappFromAgent, {
      args: [{
