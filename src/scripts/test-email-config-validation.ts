...
import { EmailConfigService } from '../temporal/services/emailConfigService';
...
async function testEmailConfigValidation() {
  ...
  for (const site of testSites) {
    ...
    const validation = EmailConfigService.validateEmailConfig(site.email);
  ...
  // Summary
  const validSites = testSites.filter(site => 
    EmailConfigService.validateEmailConfig(site.email).isValid
  );
