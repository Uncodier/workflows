...
import { deleteSchedule, createAllSchedules, defaultSchedules, listSchedules } from '../temporal/schedules';
...
async function recreateSchedules() {
  ...
  try {
    ...
    const schedulesToDelete = defaultSchedules.map(s => s.id);
