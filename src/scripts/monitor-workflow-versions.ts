...
async function monitorWorkflowVersions() {
  try {
    ...
    const connection = await Connection.connect({
      ...
      tls: temporalConfig.tls ? {} : undefined,
    });
    ...
    const activeWorkflows: WorkflowSummary[] = [];
    const completedWorkflows: WorkflowSummary[] = [];
    const failedWorkflows: WorkflowSummary[] = [];
...
export { monitorWorkflowVersions }; 