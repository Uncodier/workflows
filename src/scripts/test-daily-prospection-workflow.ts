...
import type { DailyProspectionOptions } from '../temporal/workflows/dailyProspectionWorkflow';
...
async function testDailyProspectionWorkflow() {
  try {
    ...
    const options: DailyProspectionOptions = {
      site_id: testSiteId,
    ...
    if (result.prospectionResults.length > 0) {
      console.log('\n👥 Prospection Results:');
      result.prospectionResults.forEach((prospectionResult: any, index: number) => {
...
export { testDailyProspectionWorkflow }; 