 ...
 */
async function testUnifiedStructure(): Promise<TestResult> {
  try {
    ...
    const hasRequiredFields = ['name', 'email', 'position'].every(field => 
      expectedStructure.leads[0].hasOwnProperty(field)
    );
    const hasCompanyFields = ['name', 'website', 'industry'].every(field => 
      expectedStructure.leads[0].company.hasOwnProperty(field)
    );
 ...
 */
async function testLeadExtraction(): Promise<TestResult> {
  try {
    ...
    const validExtractions = extractedLeads.filter(lead => 
 ...
 */
async function testBackwardsCompatibility(): Promise<TestResult> {
  try {
...
export { runAllTests, testUnifiedStructure, testLeadExtraction, testBackwardsCompatibility }; 