...
const deepResearchWorkflow_1 = require("../temporal/workflows/deepResearchWorkflow");
async function testDeepResearchWorkflow() {
    ...
    try {
        ...
        const handle = await client.workflow.start(deepResearchWorkflow_1.deepResearchWorkflow, {
            workflowId,
            taskQueue: 'workflows',
            args: [testOptions],
        });
        ...
        if (result.operationResults && result.operationResults.length > 0) {
            ...
            const successful = result.operationResults.filter((r) => r.success).length;
