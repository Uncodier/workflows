...
async function runNewsletterTest(client, params, scenario) {
    try {
        // Start the workflow
        const handle = await client.workflow.start(workflows_1.workflows.sendNewsletterWorkflow, {
            args: [params],
            taskQueue: 'default',
        ...
        if (result.results && result.results.length > 0) {
            ...
            result.results.slice(0, 5).forEach((emailResult, index) => {
