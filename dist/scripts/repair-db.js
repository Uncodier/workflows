...
async function repairDB() {
    ...
    for (let i = 0; i < allLeads.length; i += batchSize) {
        ...
        if (!allMessages)
            continue; // Skip to next batch if we couldn't fetch messages
        ...
        for (const lead of batch) {
            ...
            if (consecutiveAssistantCount >= 3) {
                    ...
                    while (msgUpdateRetries > 0) {
                        try {
                            const { error: updateMsgError } = await supabase
                                ...
                                .eq('id', lastMsg.id);
                            if (updateMsgError)
                                throw new Error(JSON.stringify(updateMsgError));
                        ...
                        while (leadUpdateRetries > 0) {
                            try {
                                const { error: updateLeadError } = await supabase
                                    ...
                                    .eq('id', lead.id);
                                if (updateLeadError)
                                    throw new Error(JSON.stringify(updateLeadError));
                ...
                if (!customData.sequence_stage && customData.follow_up_type === 'lead_nurture') {
                    ...
                    if (consecutiveAssistantCount === 2)
