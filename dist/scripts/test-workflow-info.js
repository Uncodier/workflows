...
try {
    ...
    // Buscar algo similar
    const similarFunctions = Object.keys(temporal).filter(key => key.toLowerCase().includes('workflow') && key.toLowerCase().includes('info'));
...
try {
    const temporal = require('@temporalio/workflow');
    const infoRelated = Object.keys(temporal).filter(key => key.toLowerCase().includes('info') ||
