...
const billingActivities_1 = require("../temporal/activities/billingActivities");
...
async function findBackfillTargets(siteIdFilter) {
    const supabase = (0, supabaseService_1.getSupabaseService)();
    ...
    for (const billing of billings) {
        if (siteIdFilter && billing.site_id !== siteIdFilter)
            continue;
        ...
        const missedCycles = (0, billingActivities_1.countMissedRenewalCycles)(new Date(cycleStart), lastRenewalDate);
        if (missedCycles === 0)
            continue;
        targets.push({
...
async function main() {
    ...
    if (siteId)
        console.log(`   Site filter: ${siteId}`);
    console.log(`   Max cycles per site: ${billingActivities_1.MAX_BACKFILL_CYCLES}`);
    console.log('');
    ...
    for (const t of targets) {
        console.log(`  - ${t.siteName} (${t.siteId}) | plan=${t.plan} | credits=${t.creditsAvailable} | missed=${t.missedCycles}`);
    }
    console.log('');
    if (dryRun) {
    ...
    for (const target of targets) {
        ...
        for (let cycle = 0; cycle < target.missedCycles; cycle++) {
            try {
                const result = await (0, billingActivities_1.renewSiteCreditsActivity)(target.siteId, target.plan, currentCredits, target.stripeSubscriptionId, { note: `Backfill credit renewal (${cycle + 1}/${target.missedCycles})` });
                currentCredits = result.newCredits;
                renewed++;
                console.log(`   ✅ Cycle ${cycle + 1}: ${result.oldCredits} -> ${result.newCredits}`);
            }
            catch (err) {
                errors++;
