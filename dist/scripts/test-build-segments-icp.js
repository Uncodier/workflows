...
async function run() {
    ...
    try {
        const handle = await client.workflow.start(workflows_1.workflows.buildSegmentsICPWorkflow, {
            taskQueue: 'default',
...
async function testSingleSegment() {
    ...
    console.log('📊 Test parameters:', JSON.stringify(testParams, null, 2));
    try {
        const handle = await client.workflow.start(workflows_1.workflows.buildSingleSegmentICPWorkflow, {
            taskQueue: 'default',
