...
exports.compareWorkflowIds = compareWorkflowIds;
...
async function compareWorkflowIds() {
    ...
    try {
        ...
        const oldPatternRecords = [];
        const newPatternRecords = [];
        cronRecords.forEach((record) => {
            ...
            const isNewPattern = !isOldPattern && (record.workflow_id.length > 60 ||
        ...
        if (newPatternRecords.length > 0) {
            ...
            const realScheduleIds = newPatternRecords.filter(r => r.schedule_id && r.schedule_id !== 'manual-execution' &&
