...
async function runTests() {
    ...
    try {
        ...
        try {
            const result1 = await (0, cronActivities_1.validateAndCleanStuckCronStatusActivity)('nonExistentWorkflow', 'test-site-id', 24);
        ...
        try {
            ...
            if (sites.length === 0) {
                ...
                for (const site of testSites) {
                    ...
                    const result = await (0, cronActivities_1.validateAndCleanStuckCronStatusActivity)('dailyStandUpWorkflow', site.id, 24);
        ...
        try {
            ...
            for (const threshold of thresholds) {
                ...
                const result = await (0, cronActivities_1.validateAndCleanStuckCronStatusActivity)('activityPrioritizationEngineWorkflow', 'global', threshold);
                console.log(`      Result: ${result.reason}`);
                console.log(`      Can proceed: ${result.canProceed}`);
                if (result.hoursStuck) {
                    console.log(`      Hours stuck: ${result.hoursStuck.toFixed(1)}`);
        ...
        try {
            ...
            if (stuckRecords.length > 0) {
                ...
                if (stuckRecords.length > 0) {
                    ...
                    const cleanupResult = await (0, cronActivities_1.validateAndCleanStuckCronStatusActivity)(testRecord.activity_name, testRecord.site_id, 1 // 1 hour threshold to force cleanup
                    );
