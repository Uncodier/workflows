...
exports.verifyCronStatusIds = verifyCronStatusIds;
...
async function verifyCronStatusIds() {
    ...
    try {
        ...
        const analysis = {
            ...
            withScheduleId: 0,
            withBothIds: 0,
            workflowIdPatterns: new Set(),
            scheduleIdPatterns: new Set(),
            activityNames: new Set()
        };
        console.log('📋 ANÁLISIS DETALLADO:');
        console.log('-'.repeat(40));
        cronRecords.forEach((record, index) => {
        ...
        expectedScheduleIds.forEach(expectedId => {
            const found = cronRecords.some((record) => record.schedule_id === expectedId);
        ...
        const customWorkflowPatterns = Array.from(analysis.workflowIdPatterns).filter(pattern => pattern.includes('${workflow-name}-${site-id}') ||
        ...
        const fallbackSchedulePatterns = Array.from(analysis.scheduleIdPatterns).filter(pattern => pattern.includes('fallback') ||
