...
exports.MAX_BACKFILL_CYCLES = void 0;
exports.countMissedRenewalCycles = countMissedRenewalCycles;
exports.fetchSitesDueForCreditRenewalActivity = fetchSitesDueForCreditRenewalActivity;
exports.renewSiteCreditsActivity = renewSiteCreditsActivity;
exports.fetchSitesNeedingInitializationActivity = fetchSitesNeedingInitializationActivity;
exports.initializeSiteCreditsActivity = initializeSiteCreditsActivity;
const supabaseService_1 = require("../services/supabaseService");
const MAX_BACKFILL_CYCLES = 12;
exports.MAX_BACKFILL_CYCLES = MAX_BACKFILL_CYCLES;
/** Count billing-cycle renewal dates that have passed but were never credited. */
function countMissedRenewalCycles(cycleStartDate, lastRenewalDate, today = new Date()) {
...
async function fetchSitesDueForCreditRenewalActivity() {
    ...
    for (const billing of billings) {
        ...
        const startedToday = startDate.getUTCFullYear() === today.getUTCFullYear() &&
        ...
        if (lastRenewal) {
            ...
            const daysSinceLastRenewal = (today.getTime() - lastRenewalDate.getTime()) / (1000 * 60 * 60 * 24);
            if (isDue) {
...
async function renewSiteCreditsActivity(siteId, plan, currentCredits, stripeSubscriptionId, options) {
    ...
    try {
        ...
        if (stripeSubscriptionId) {
            const lastStripePayment = await supabaseService.fetchLastStripeSubscriptionPayment(siteId, stripeSubscriptionId);
...
async function initializeSiteCreditsActivity(siteId) {
    ...
    try {
        ...
        if (normalizedPlan === 'startup')
            ...
            initialCredits = 1000;
        else if (normalizedPlan === 'commission')
            initialCredits = 20;
        if (!existingBilling) {
