...
exports.fetchSitesActivity = fetchSitesActivity;
exports.scheduleEmailSyncWorkflowsActivity = scheduleEmailSyncWorkflowsActivity;
exports.updateCronStatusActivity = updateCronStatusActivity;
 ...
 */
async function fetchSitesActivity(options = {}) {
    console.log('📂 Fetching sites with email sync enabled...');
    // Validate scheduling options
    const optionsValidation = services_1.EmailSyncSchedulingService.validateSchedulingOptions(options);
    ...
    try {
        ...
        const sites = allSitesData.map(siteRow => {
            const site = {
                id: siteRow.id,
            ...
            if (siteSettings) {
                const emailConfig = services_1.EmailConfigService.extractEmailConfigFromSettings(siteSettings);
                if (emailConfig) {
        ...
        const sitesWithStatus = services_1.EmailSyncSchedulingService.processSitesForScheduling(sites, cronData || [], { ...options, minHoursBetweenSyncs: 1 } // Email sync every hour
        );
        // Log detailed analysis
        services_1.EmailSyncSchedulingService.logSchedulingAnalysis(sitesWithStatus, options);
...
async function fetchMockSitesData(options = {}) {
    ...
    // Process sites for scheduling using the scheduling service
    const sitesWithStatus = services_1.EmailSyncSchedulingService.processSitesForScheduling(sitesWithEmail, mockCronStatuses, options);
    // Log detailed analysis
    services_1.EmailSyncSchedulingService.logSchedulingAnalysis(sitesWithStatus, options);
    return sitesWithStatus;
}
 ...
 */
async function scheduleEmailSyncWorkflowsActivity(sites, options = {}) {
    ...
    for (const site of sites) {
        ...
        try {
            ...
            if (site.email) {
                const provider = services_1.EmailConfigService.getEmailProvider(site.email);
 ...
 */
async function updateCronStatusActivity(updates) {
