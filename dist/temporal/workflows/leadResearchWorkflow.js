...
function generateLeadResearchQuery(lead) {
    ...
    if (lead.company || lead.company_name) {
        ...
        if (typeof lead.company === 'object' && lead.company !== null) {
            ...
            const companyInfo = [];
            if (lead.company.industry && !lead.industry)
                companyInfo.push(`industria: ${lead.company.industry}`);
            if (lead.company.size && !lead.company_size)
                companyInfo.push(`tamaño: ${lead.company.size}`);
            if (lead.company.location)
                companyInfo.push(`ubicación empresa: ${lead.company.location}`);
